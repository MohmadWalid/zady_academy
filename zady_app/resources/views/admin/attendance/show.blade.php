...
<div class="flex items-center gap-4 mb-8">
    <a href="{{ route('admin.attendance.index', ['date' => $date]) }}" class="w-10 h-10 flex items-center justify-center rounded-full bg-surface border border-border text-text-secondary hover:text-primary transition-colors">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
    </a>
    <x-page-header title="{{ $group->name }}" subtitle="تحضير الطلاب ليوم {{ $date }}" />
</div>

<form action="{{ route('admin.attendance.store', $group) }}" method="POST" id="attendance-form">
    @csrf
