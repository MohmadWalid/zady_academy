...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    ...
    <div class="flex flex-col md:flex-row gap-2 w-full sm:w-auto">
        <form action="{{ route('admin.attendance.index') }}" method="GET" class="flex gap-2">
            <x-search-bar name="search" value="{{ request('search') }}" placeholder="ابحث باسم الحلقة..." />
            <input type="hidden" name="date" value="{{ $date }}">
            <input type="date" name="date" value="{{ $date }}" onchange="this.form.submit()" class="h-[48px] px-4 border border-border rounded-[24px] bg-surface text-sm focus:ring-primary focus:border-primary outline-none">
        </form>
    </div>
</div>
...
@else
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($groups as $group)
            <a href="{{ route('admin.attendance.show', ['group' => $group, 'date' => $date]) }}" class="block hover:-translate-y-1 transition-transform">
