...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    ...
    <div class="flex items-center gap-3 w-full sm:w-auto">
        ...
        <a href="{{ route('admin.academic.groups.create') }}" class="bg-primary text-white px-6 py-2 rounded-xl font-bold hover:bg-primary/90 transition-all flex items-center gap-2 whitespace-nowrap">
...
@if($groups->isEmpty())
    <x-card>
        <x-empty-state 
            ...
            actionRoute="{{ route('admin.academic.groups.create') }}" />
    </x-card>
@else
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($groups as $group)
            <a href="{{ route('admin.academic.groups.show', $group) }}" class="block hover:-translate-y-1 transition-transform">
        ...
        @foreach($archived as $oldGroup)
            <div class="relative group">
                ...
                <div class="absolute inset-0 bg-surface/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-[14px]">
                    <form action="{{ route('admin.academic.groups.restore', $oldGroup->id) }}" method="POST">
                        @csrf
