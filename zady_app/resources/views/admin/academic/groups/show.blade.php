...
<div class="flex items-center gap-4 mb-8">
    <a href="{{ route('admin.academic.groups.index') }}" class="w-10 h-10 flex items-center justify-center rounded-full bg-surface border border-border text-text-secondary hover:text-primary transition-colors">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-1 flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-2">
                    @foreach($group->sessions as $session)
                        <div class="flex justify-between items-center p-3 rounded-lg bg-bg/50 border border-border">
                            ...
                            <form action="{{ route('admin.academic.groups.sessions.destroy', [$group, $session]) }}" method="POST" onsubmit="return confirm('حذف هذا الموعد؟')">
                                @csrf
                                @method('DELETE')
        ...
        <x-card>
            ...
            <div class="space-y-4">
                <div class="flex flex-col gap-1">
                    ...
                    <span class="text-xs font-medium text-text-primary">{{ $group->creator->name ?? 'النظام (Seeder)' }}</span>
                    <span class="text-[10px] text-text-secondary">{{ $group->created_at->format('Y-m-d H:i') }}</span>
                </div>
                
                @if($group->updated_by)
                    <div class="flex flex-col gap-1">
                        <span class="text-[10px] text-text-secondary font-bold uppercase">آخر تحديث بواسطة</span>
                        <span class="text-xs font-medium text-text-primary">{{ $group->updater->name ?? '---' }}</span>
                        <span class="text-[10px] text-text-secondary">{{ $group->updated_at->format('Y-m-d H:i') }}</span>
                    </div>
                @endif
            </div>
        </x-card>
        ...
        <div class="flex flex-col gap-3">
            <a href="{{ route('admin.academic.groups.edit', $group) }}" class="w-full bg-surface border border-border text-text-primary py-3 rounded-xl font-bold hover:bg-bg transition-all flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                تعديل المجموعة
            </a>
            <form action="{{ route('admin.academic.groups.destroy', $group) }}" method="POST" onsubmit="return confirm('نقل للمجموعة للأرشيف؟')">
                @csrf
                @method('DELETE')
    ...
    <div class="lg:col-span-2 flex flex-col gap-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($group->activeEnrollments as $enrollment)
                        <div class="flex items-center justify-between p-3 rounded-xl bg-bg border border-border">
                            ...
                            <a href="{{ route('admin.academic.students.show', $enrollment->student) }}" class="text-xs text-primary font-bold">الملف →</a>
                        </div>
                    @endforeach
                </div>
            @endif
        ...
        <x-card>
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('admin.attendance.show', $group) }}" class="text-sm font-bold text-primary">عرض الكل</a>
            </div>
...
<div id="add-session-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center hidden p-4">
    <div class="bg-surface rounded-2xl w-full max-w-md shadow-2xl p-6">
        ...
        <form action="{{ route('admin.academic.groups.sessions.store', $group) }}" method="POST" class="flex flex-col gap-6">
            @csrf
            <div>
