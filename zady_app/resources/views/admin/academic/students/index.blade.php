...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    ...
    <div class="flex items-center gap-3 w-full sm:w-auto">
        ...
        <a href="{{ route('admin.academic.students.create') }}" class="bg-primary text-white px-6 py-2 rounded-xl font-bold hover:bg-primary/90 transition-all flex items-center gap-2 whitespace-nowrap">
...
<x-card class="overflow-hidden">
    @if($students->isEmpty())
        <x-empty-state 
            ...
            actionRoute="{{ route('admin.academic.students.create') }}" />
    @else
                ...
                <tbody class="divide-y divide-border">
                    @foreach($students as $student)
                        <tr class="hover:bg-bg transition-colors">
                            <td class="px-6 py-4">
                                <a href="{{ route('admin.academic.students.show', $student) }}" class="font-bold text-text-primary hover:text-primary transition-colors">
                                    {{ $student->name }}
                                </a>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.academic.students.edit', $student) }}" class="p-2 text-text-secondary hover:text-primary transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form action="{{ route('admin.academic.students.destroy', $student) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا الطالب؟');">
                                        @csrf
                                        @method('DELETE')
                ...
                <tbody class="divide-y divide-border">
                    @foreach($archived as $oldStudent)
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <form action="{{ route('admin.academic.students.restore', $oldStudent->id) }}" method="POST">
                                    @csrf
