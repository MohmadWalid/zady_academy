...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    ...
    <form action="{{ route('secretary.payments.pending') }}" method="GET" class="w-full sm:w-80">
            ...
            <x-card class="flex flex-col gap-4 relative overflow-hidden">
                ...
                @if($payment->proof_image)
                    <div class="bg-bg rounded-lg p-2 text-center border border-border">
                        <a href="{{ route('proofs.show', $payment->payment_code) }}" target="_blank" class="text-primary text-sm font-medium flex items-center justify-center gap-2 hover:underline">
                ...
                <div class="flex gap-2 mt-2">
                    <form action="{{ route('secretary.payments.approve', $payment) }}" method="POST" class="flex-1" onsubmit="return confirm('هل أنت متأكد من قبول هذه الدفعة؟')">
                        @csrf
...
<script>
    function openRejectModal(id, code) {
        ...
        form.action = `/secretary/payments/${id}/reject`;
