...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('secretary.academic.students.show', $student) }}" class="w-10 h-10 flex items-center justify-center rounded-full bg-surface border border-border text-text-secondary hover:text-primary transition-colors">
    ...
    <x-card>
        <form action="{{ route('secretary.academic.students.update', $student) }}" method="POST" class="flex flex-col gap-6">
            @csrf
            @method('PUT')
            ...
            <div class="pt-4 flex gap-4">
                ...
                <a href="{{ route('secretary.academic.students.show', $student) }}" class="flex-1 bg-surface border border-border text-text-secondary h-12 rounded-xl font-bold flex items-center justify-center hover:bg-bg transition-all">إلغاء</a>
            </div>
        </form>
    </x-card>
</div>
